...
class PitchTable extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      showDetail: false
  ...
  render() {
    ...
    const data = this.props.pitches.data.map((pitch, index) => (
      [index+1, pitch.title, pitch.creator_id, new Date(pitch.updated_at).toLocaleDateString(), pitch.status,  pitch.approve_by_admin, detailButton]
    ))
              ...
              <NavPills
                ...
                tabs={[
                  ...
                  },
                  {
                    ...
                    tabIcon: HelpOutline,
                    tabContent: (
                      <Timeline simple stories={progress_data} />
                    )
                  }
                ]}
              />
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withStyles(extendedTablesStyle)(PitchTable));
